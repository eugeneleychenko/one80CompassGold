...
const ChatInterface = () => {
    ...
    useEffect(() => {
      if (hasFriendResponded && messages.some(message => message.sender === 'friend')) {
    ...
    useEffect(() => {
        if (!hasUserSentFirstMessage && messages.some(message => message.sender === 'user')) {
  ...
  return (
    ...
    <Grid container sx={{ height: '100%', position: 'relative' }}>
      <Grid item xs={12} sx={{ height: 'calc(100% - 80px)', overflowY: 'auto', paddingBottom: '180px' }}>
        <List sx={{ padding: 0 }}>
            ...
            <React.Fragment key={index}>
                    ...
                    message.details && message.details.AI_responses ? (
                      message.details.AI_responses.map((response, idx) => (
                        <React.Fragment key={idx}>
                          ...
                          {idx < message.details.AI_responses.length - 1 && <br />}
                        </React.Fragment>
                      ))
                    ) : (
              ...
              {message.sender === 'friend' && message.details && !hasFriendResponded && (
      ...
      <Grid item xs={12} sx={{ position: 'absolute', bottom: 0, left: 0, right: 0 }}>
